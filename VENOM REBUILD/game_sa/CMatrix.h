...
public:
	...
	CMatrix Rotate ( const CVector * param, float theta )
	{
		...
		CMatrix mRotateMult;
		// rotate X
		mRotateMult.vRight.fX = (float)cos_t + ( 1.0f - (float)cos_t ) * param->fX * param->fX;
		mRotateMult.vRight.fY =  ( 1.0f - (float)cos_t ) * param->fX * param->fY - (float)sin_t * param->fZ;
		mRotateMult.vRight.fZ =  ( 1.0f - (float)cos_t ) * param->fX * param->fZ + (float)sin_t * param->fY;
		// rotate Y
		mRotateMult.vFront.fX =  ( 1.0f - (float)cos_t ) * param->fY * param->fX + (float)sin_t * param->fZ;
		mRotateMult.vFront.fY = (float)cos_t + ( 1.0f - (float)cos_t ) * param->fY * param->fY;
		mRotateMult.vFront.fZ =  ( 1.0f - (float)cos_t ) * param->fY * param->fZ - (float)sin_t * param->fX;
		// rotate Z
		mRotateMult.vUp.fX =  ( 1.0f - (float)cos_t ) * param->fZ * param->fX - (float)sin_t * param->fY;
		mRotateMult.vUp.fY = ( 1.0f - (float)cos_t ) * param->fZ * param->fY + (float)sin_t * param->fX;
		mRotateMult.vUp.fZ = (float)cos_t + ( 1.0f - (float)cos_t ) * param->fZ * param->fZ;
		// multiply matrix
